...
<script>
    ...
    export default {
        name: "shop-shops",
        mounted(){
          //如果从自身页面刷新的话,数据肯定还没来,就不会执行initScroll
          //如果从评价或者点餐界面跳转过来的话,info数据已经存在(因为header已经存在了),就可以调用initScroll
          if(!this.info.name){
            return;
          }
          this._initScroll()
        ...
        watch:{
          //当在自身页面刷新的时候等待数据更新后再进行调用initScroll
          info(){
            this.$nextTick(()=>{
              this._initScroll()
            })
